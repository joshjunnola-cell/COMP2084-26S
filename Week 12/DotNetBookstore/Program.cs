...
// It must come BEFORE UseRouting() so static files are served without routing.
app.UseStaticFiles();

// Routing middleware determines the endpoint that will handle the request.
app.UseRouting();

// Enable session middleware so controllers/pages can read/write session data.
// Must be placed after UseRouting and before endpoint execution.
app.UseSession();
